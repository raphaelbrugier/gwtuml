...
public abstract class RelationLinkArtifact extends LinkArtifact {
	...
	case COMPOSITION:
	    return new CompositionLinkArtifact(left, right);
	case DEPENDENCY:
	    return new DependencyLinkArtifact(left, right);
	case GENERALIZATION:
	    return new GeneralizationLinkArtifact(left, right);
	case ASSOCIATION:
	    return new AssociationLinkArtifact(left, right);
	case REALIZATION:
	    return new RealizationLinkArtifact(left, right);
	default:
	    return new AggregationLinkArtifact(left, right);
	}
	...
	if (this.leftClassArtifact != this.rightClassArtifact) {
	    ...
	    GfxManager.getPlatform().moveTo(this.line, linePoints.get(0).getX(),
		    linePoints.get(0).getY());
	    GfxManager.getPlatform().lineTo(this.line, linePoints.get(1).getX(),
		    linePoints.get(1).getY());
	    GfxManager.getPlatform().lineTo(this.line, linePoints.get(2).getX(),
		    linePoints.get(2).getY());
	    GfxManager.getPlatform().lineTo(this.line, linePoints.get(3).getX(),
		    linePoints.get(3).getY());
	    GfxManager.getPlatform().lineTo(this.line, linePoints.get(4).getX(),
		    linePoints.get(4).getY());
	...
	if (editPart == null) {
	    ...
	    editor
		    .startEdition(editPart.getText(this.relation), GfxManager
			    .getPlatform().getXFor(editedGfxObject), GfxManager
			    .getPlatform().getYFor(editedGfxObject)
	...
	if (point.getX() == classArtifact.getX()) {
	    return Anchor.LEFT;
	} else if (point.getY() == classArtifact.getY()) {
	    return Anchor.TOP;
	} else if (point.getX() == classArtifact.getX()
