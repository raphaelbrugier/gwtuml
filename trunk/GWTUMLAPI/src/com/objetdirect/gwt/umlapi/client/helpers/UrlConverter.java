...
public class UrlConverter {
	...
	public void fromURL(final String url, final boolean isForPasting) {
		if (!url.equals("AA==")) {
			...
			Point pasteShift = isForPasting ? Point.substract(Point.substract(getCurrentMousePosition(), getCopyMousePosition()), umlCanvas.getCanvasOffset()) : Point.getOrigin();
				...
				if (!artifactWithParameters.equals("")) {
					...
					if (artifactAndParameters.length > 1) {
						...
						if (artifact.equals("Class")) {
							newArtifact = new ClassArtifact(umlCanvas, id, (isForPasting && wasACopy() ? "CopyOf" : "") +UMLClass.parseNameOrStereotype(parameters[1]), UMLClass.parseNameOrStereotype(parameters[2]));
							...
							newArtifact = new ObjectArtifact(umlCanvas, id, UMLObject.parseName(parameters[1]).get(0), (isForPasting && wasACopy() ? "CopyOf" : "") + UMLObject.parseName(parameters[1]).get(1),
							...
							newArtifact = new LifeLineArtifact(umlCanvas, id, (isForPasting && wasACopy() ? "CopyOf" : "") + UMLLifeLine.parseName(parameters[1]).get(1), UMLLifeLine.parseName(parameters[1]).get(0));
							newArtifact.setLocation(Point.add(Point.parse(parameters[0]), pasteShift));
