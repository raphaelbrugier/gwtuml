...
public class NoteEditor {
	...
	public void editContent() {
		...
		editedNote.getCanvas().add(editField,
				editedNote.getX() + FIELD_XMARGIN,
		...
		KeyboardListener keybLst = new KeyboardListener() {
			...
			public void onKeyUp(Widget sender, char keyCode, int modifiers) {
				...
				if (keyCode == KEY_PAGEDOWN)
					goNextBox();
				/*
				 * if (keyCode==KEY_UP) { if ((modifiers&MODIFIER_SHIFT)!=0)
				 * moveUp(); else goUp(); }
				 */
				if (keyCode == KEY_DOWN) {
					/*
					 * if ((modifiers&MODIFIER_SHIFT)!=0) moveDown(); else
					 */
					goDown();
