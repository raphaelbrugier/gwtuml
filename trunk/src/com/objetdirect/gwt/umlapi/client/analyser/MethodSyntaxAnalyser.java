...
public class MethodSyntaxAnalyser extends SyntaxAnalyser  {
	...
	protected LexicalAnalyser.Token processToken(LexicalAnalyser lex, LexicalAnalyser.Token tk) {
		if (tk==null)
			tk=lex.getToken();
		switch (getStatus()) {
		case BEGIN:
		...
		case OPEN_PARENTHESIS_EXPECTED:
			if (tk==null)
				throwUnexpectedEOF();
			else if (tk.getType()==LexicalAnalyser.SIGN && tk.getContent().equals("(")) {
		...
		case BEGIN_RETURN_TYPE:
			setParameters();
			if (tk!=null && tk.getType()==LexicalAnalyser.SIGN && tk.getContent().equals(":")) {
